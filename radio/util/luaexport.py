...
def LEXP_TELEMETRY(name, description, scale = 1, transform = None):
  #print "LEXP %s, %s, %s, %s" % (name, description, scale, transform)
  checkName(name)
  exports.append( ("MIXSRC_FIRST_TELEM-1+"+CONSTANT_VALUE, name, description, scale, transform) )
...
data = ["  {%s, \"%s\", \"%s\"}" % (cnst, name, desc) for (cnst, name, desc, scale, transf) in exports]
out.write(",\n".join(data))
out.write("\n};\n\n")
